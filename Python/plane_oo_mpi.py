...
class EscapedHistogram:
    ...
    def write_to_file(self):
        ...
        with open("intensity.txt", "w", encoding="utf-8") as file_out:
            file_out.write("angle weight intensity\n")
            for n in range(self.n_bins):
                file_out.write(f"{self.theta[n]} {self.weight[n]} {self.intensity[n]}\n")
...
def main():
    ...
    n_photons = int(1e5)
    n_photons_per_process = n_photons // n_ranks
    ...
    n_start = rank * n_photons_per_process
    n_stop = (rank + 1) * n_photons_per_process
    ...
    for i in range(n_start, n_stop):
        ...
        if i % output_freq == 0 and rank == 0:
            percent_complete = 100 * i / n_photons_per_process
            print("{:3.1f}% photons transported.".format(percent_complete))

    # comm.Reduce([hist.weight, MPI.FLOAT], [hist.weight, MPI.FLOAT], op=MPI.SUM, root=0)
    comm.Allreduce(MPI.IN_PLACE, hist.weight, op=MPI.SUM)
    ...
    if rank == 0:
        ...
        ax.plot(theta_bins, intensity, "kx")
